...
![[assets/the_spring_ioc_container 1.png]]
...
```
⚠️  스프링 IoC 컨테이너에 대해 배운 후에는, URI 구문에 정의된 위치에서 입력 스트림을 읽기 위한 편리한 메커니즘을 제공하는 스프링의 `Resource` 추상화('리소스'에 설명된 대로)에 대해 자세히 알아보고 싶을 수 있다. 특히 `Resource` 경로는 '애플리케이션 컨텍스트 및 리소스 경로'에 설명된 대로 애플리케이션 컨텍스트를 생성하는 데 사용된다.
```
